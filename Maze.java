...
public class Maze {
	...
	public Maze(int horizontalSize, int verticalSize) {
		...
		//cell(x, y) is in cells[x + y*h]
		DisjointSets cells = new DisjointSets(horiz*vert);

		int vTotal = (horiz - 1) * vert;
		int hTotal = horiz*(vert - 1);

		int[] walls = new int[ vTotal + hTotal];

		for(int k = 0; k < hTotal + vTotal; k++){
			walls[k] = k;
		}

		for(int w =  vTotal + hTotal - 1; w > 0; w--){
			int r = randInt(w);
			int temp = walls[r];
			walls[r] = walls[w];
			walls[w] = temp;
		}
		//vertical [x][y] is in walls[x + y*(h-1)]
		//horizontal [x][y] is in walls[x + y*h + vTotal]
		int x, y;
		for(int k = 0; k < walls.length; k++){
			//from vTotal ~ vTotal+hTotal - 1, are horizontal walls
			//horizontal wall[x][y] separates (x, y) and (x, y+1)
			if(walls[k] >= vTotal){
				x = (walls[k] - vTotal) % horiz;
				y = (walls[k] - vTotal) / horiz;
				int upperCell = x + y * horiz;
				int bottomCell = x + (y + 1)* horiz;

				int rootUpper = cells.find(upperCell);
				int rootBottom =  cells.find(bottomCell);
				if(rootUpper == rootBottom){
					continue;
				}else{
					hWalls[x][y] = false;
					cells.union(rootUpper, rootBottom);
				}

				//from 0 ~ vTotal - 1, are vertical walls
				//vertical walls[x][y] separates(x, y) and(x+1, y)
			}else{
				x = walls[k]%(horiz - 1);
				y = walls[k]/(horiz - 1);
				int leftCell = x  + y * horiz;
				int rightCell = x + 1 + y *horiz;

				int rootLeft = cells.find(leftCell);
				int rootRight =  cells.find(rightCell);

				if(rootLeft == rootRight ){
					continue;
				} else {
					vWalls[x][y] = false;
					cells.union(rootLeft, rootRight);
				}
			}
		}
	}
